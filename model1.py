...
import os
os.chdir('d:/大三/專題/project')
# 讀取數據和數據預處理
data = pd.read_csv('new_spx_40.csv')
...
# 構建 LSTM 模型
model = Sequential()
model.add(LSTM(30,activation='relu', input_shape=(look_back, X_train.shape[2],), kernel_regularizer=l2(0.1),return_sequences=True))
model.add(Dropout(0.75))
model.add(LSTM(30, activation='relu', kernel_regularizer=l2(0.1)))
model.add(Dropout(0.75))
model.add(Dense(25))
